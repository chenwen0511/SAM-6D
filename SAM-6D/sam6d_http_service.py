...
logger = logging.getLogger("sam6d_http")
...
def _startup_warmup() -> None:
    """Prepare template cache and preload in-process models.

    PEM/ISM still run in a fresh subprocess per /infer, so their weight load cost remains each call;
    warming YOLO + templates removes first-hit latency after process start.
    """
    cad_raw = os.environ.get("SAM6D_CAD_PATH")
    if cad_raw:
        cad_path = Path(cad_raw).expanduser().resolve()
        if cad_path.is_file():
            try:
                _ensure_templates(cad_path)
                logger.info("Warmup: templates ready for %s", cad_path)
            except Exception:
                logger.exception("Warmup: template preparation failed (service still starts)")
        else:
            logger.warning("Warmup: SAM6D_CAD_PATH does not exist, skip templates")
    else:
        logger.warning("Warmup: SAM6D_CAD_PATH unset, skip templates")

    backend = os.environ.get("SAM6D_SEG_BACKEND", "sam6d_ism")
    yw = os.environ.get("SAM6D_YOLO_WEIGHTS")
    if backend == "yolo_seg" and yw:
        wp = Path(yw).expanduser().resolve()
        if wp.is_file():
            try:
                from yolo_seg_backend import _load_model

                _load_model(wp)
                logger.info("Warmup: YOLO weights loaded from %s", wp)
            except Exception:
                logger.exception("Warmup: YOLO preload failed (service still starts)")
        else:
            logger.warning("Warmup: SAM6D_YOLO_WEIGHTS missing, skip YOLO preload")


@asynccontextmanager
async def _lifespan(_: FastAPI):
    logging.basicConfig(level=logging.INFO)
    await asyncio.to_thread(_startup_warmup)
    yield
...
def _run_sam6d_pipeline(
    ...
    _run_command(
        [
            ...
            str(seg_path),
            "--det_score_thresh",
            str(pem_det_score_thresh),
        ],
        cwd=ROOT_DIR / "Pose_Estimation_Model",
...
app = FastAPI(title="SAM-6D HTTP Service", lifespan=_lifespan)
...
async def warmup() -> Dict[str, str]:
    """Re-run startup warmup under the infer lock (e.g. after changing SAM6D_CAD_PATH)."""
